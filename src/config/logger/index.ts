...
DbLogger.prototype.log = (level: any, msg: any, meta: any, callback: Function) => {
  try {
    ...
    const logModel = new LogSchema({
      service: 'Routing controller ts server',
      message: msg,
      user: meta.user,
      type: type,
...
logger.rewriters.push(function (level: any, msg: any, meta: any) {
  if (meta && meta.request) {
    meta.user = meta.request.currentUser && meta.request.currentUser._id ? meta.request.currentUser._id.toString() : undefined;
    meta.organization = meta.request.currentOrganization && meta.request.currentOrganization._id ?
      meta.request.currentOrganization._id.toString() :
      undefined;
